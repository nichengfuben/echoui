...
FILENAME_EXEMPT_NAMES: Set[str] = {"__init__.py", "conftest.py", "__main__.py", "docker-compose.yml", "docker-compose.yaml"}
...
class Scanner:
    ...
    def scan_dir_violations(self) -> List[DirViolation]:
        ...
        for dirpath, _dirnames, _filenames in _walk_dirs(self.root):
            ...
            elif len(children) < MIN_CHILDREN:
                violations.append(
                    DirViolation(
                        path=current,
                        child_count=len(children),
...
class ReportGenerator:
    ...
    def generate(self) -> str:
        ...
        lines.extend([
            "=== 文件命名 ===",
            "约束: 文件名(不含扩展名)不得含 - . emoji mixin 数字，下划线最多1个",
