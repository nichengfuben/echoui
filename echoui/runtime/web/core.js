...
document.addEventListener("DOMContentLoaded",function(){var c=window.__ECHoui_CFG||{};var r=resumeCfg();if(r&&r.signals)Object.keys(r.signals).forEach(function(k){c.signals=c.signals||{};if(c.signals[k]===undefined)c.signals[k]=r.signals[k];});Object.keys(c.signals||{}).forEach(function(k){S[k]=c.signals[k];});if(window.__echoui.storage&&window.__echoui.storage.opfsInit)window.__echoui.storage.opfsInit();var ui=window.__echoui&&window.__echoui.ui;(c.bindings||[]).forEach(function(b){if(b.t==="text")bindT(b.n,b.tpl,b.d);else if(b.t==="style")bindS(b.n,b.p,b.v,b.d);else if(b.t==="attr"&&ui)ui.bindAttr(b.n,b.a,b.v,b.d,S,sub);else if(b.t==="bg"&&ui)ui.bindBg(b.n,b.v,b.d,S,sub);});if(ui){ui.wireFiles(c.file_inputs,S,s,sub);ui.wireOverlays(c.overlays,S,sub);}(c.clicks||[]).forEach(function(x){clk(x.node,c.actions[x.action||x.handler]||x.handler);});wireD(c.dom);if(c.frame_script)frameFn=loadFrameScript(c.frame_script);gpu();fitStage();window.addEventListener("resize",fitStage);if(c.frames||c.frame_local)requestAnimationFrame(loop);});
})();
