...
var ime={active:false,data:""};
function onComposition(type,e){
  if(type==="start"){ime.active=true;ime.data=e.data||"";}
  else if(type==="update"){ime.active=true;ime.data=e.data||"";}
  else{ime.active=false;ime.data=e.data||"";if(ime.data&&window.__echoui&&window.__echoui.onCommit)window.__echoui.onCommit(ime.data);}
  s("__ime_active",ime.active);s("__ime_data",ime.data||"");
}
document.addEventListener("compositionstart",function(e){onComposition("start",e);});
document.addEventListener("compositionupdate",function(e){onComposition("update",e);});
document.addEventListener("compositionend",function(e){onComposition("end",e);});
window.__echoui={g:g,s:s,sub:sub,apply:function(p){Object.keys(p).forEach(function(k){s(k,p[k]);});},key:function(c){return!!keys[c];},composing:function(){return!!ime.active;},compositionText:function(){return ime.data||"";},gpu:gpu,fitStage:fitStage,storage:function(){return window.__echoui.storage;}};
